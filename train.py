...
model_cfg = dict(
    ...
    eval_period=5,  # 评估周期
    # ---------- 消融实验超参数 ----------
    deploy=True,  # 结构重参数化多分支结构
    aspp_module=False,  # ASPP模块
    level_fusion=True,  # 深浅层次特征融合
)
...
def main(model_cfg):
    ...
    # ---------- 消融实验超参数 ----------
    deploy, aspp_module, level_fusion = (
        model_cfg["deploy"],
        model_cfg["aspp_module"],
        model_cfg["level_fusion"],
    )
    # ---------- 实例化卷积神经网络模型 ----------
    model = DeepLab(
        num_classes, backbone, downsample_factor, aux_branch, aspp_module, level_fusion
    )
    if deploy:
